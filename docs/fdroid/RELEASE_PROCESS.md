...
Copy-paste-friendly playbook for every future release, given the state established by the initial F-Droid submission.
...
- [x] Fork-specific signing key (ZDarow, SHA-256 in recipe)
- [x] `gh` CLI authenticated for GitHub releases

## Release variables

Fill these in before starting. Using `0.1.0-alpha-2` as the example — bump for each new release.

```
NEW_VERSION_NAME=0.1.0-alpha-2
...
Create `fastlane/metadata/android/en-US/changelogs/N.txt`. **≤ 500 characters.** Plain text, no markdown. Describe user-visible changes.

### 3. Commit, tag, push

```bash
git add app/build.gradle.kts fastlane/metadata/android/en-US/changelogs/N.txt
git commit -m "release: помечать версию $NEW_VERSION_NAME (versionCode $NEW_VERSION_CODE)"
git tag Equalizer314-v$NEW_VERSION_NAME
git push
git push origin Equalizer314-v$NEW_VERSION_NAME
```
...
**Expected fingerprint:** — проверьте по `AllowedAPKSigningKeys` в рецепте.
...
cp app/build/outputs/apk/release/app-release.apk \
   app/build/outputs/apk/release/Equalizer314-v$NEW_VERSION_NAME.apk

gh release create "Equalizer314-v$NEW_VERSION_NAME" \
  --title "Equalizer314-v$NEW_VERSION_NAME" \
  --notes "<short release notes>" \
  app/build/outputs/apk/release/Equalizer314-v$NEW_VERSION_NAME.apk
```

The `Binaries:` URL pattern `…/releases/download/Equalizer314-v%v/Equalizer314-v%v.apk` resolves to this artifact via `%v = $NEW_VERSION_NAME`. F-Droid will curl it during the reproducibility check.

### 6. Update the fdroiddata MR

Option A — **auto-update**. Because the recipe has `AutoUpdateMode: Version` + `UpdateCheckMode: Tags`, F-Droid's checkupdate bot will see the new tag and automatically open a follow-up MR to append a new `Builds:` entry. You don't need to do anything else.
...
git commit -m "Equalizer314: bump to $NEW_VERSION_NAME"

GL_TOKEN='<your-gitlab-pat>'
git push "https://oauth2:${GL_TOKEN}@gitlab.com/<user>/fdroiddata.git" <branch>
```
